...
// Apne backend ka URL yahan daal (local network ya deployed)
const SOCKET_URL = "http://localhost:5000"; 
let socket;

const Chat = () => {
  const { peerId } = useParams(); // Jisse chat karni hai uski ID
  ...
  useEffect(() => {
    ...
    // 2. Apna personal chat room join karo
    if (myId) {
      socket.emit("join_chat", myId);
    }

    // 3. Purani chat history fetch karo
    fetchChatHistory();

    // 4. Peer (doosre bande) ki details fetch karo (naam dikhane ke liye)
    fetchPeerDetails();

    // 5. Naye messages listen karo
  ...
  const fetchPeerDetails = async () => {
    try {
      // Assuming you have an endpoint to get basic user details by ID
  ...
  const handleSendMessage = (e) => {
    ...
    // Socket ke through bhej do
    socket.emit("send_message", msgData);

    // Apni screen par turant add kar lo (fake it till you make it)
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 max-w-md mx-auto shadow-xl relative">
      ...
      {/* HEADER */}
      <div className="bg-orange-600 text-white px-4 py-3 flex items-center gap-3 shadow-md z-10">
        <button onClick={() => navigate(-1)} className="p-1 hover:bg-orange-500 rounded-full cursor-pointer transition">
          <ArrowLeft size={20} />
        </button>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.map((msg, index) => {
          // 🔥 YAHAN FIX KIYA HAI: Ab ye MongoDB ke object ID ko bhi theek se samajh lega
